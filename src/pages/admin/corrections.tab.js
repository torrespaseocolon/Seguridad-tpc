...
function openEditModal(session, reload) {
  ...
  const destInput = el("input", { class: "form-control", value: session.destinationNumber || "" });
  ...
  const content = el("div", { class: "stack" }, [
    ...
    field("Número de apartamento/oficina", destInput),
    ...
    try {
      await correctSessionFields(
        session.id,
        {
          ...
          destinationNumber: destInput.value.trim(),
        },
